...
window.addEventListener('DOMContentLoaded', () => {
  ...
  fetchSupabaseAlbum().then(supabaseAlbum => {
    library = supabaseAlbum ? [supabaseAlbum] : [];
    publishLibrary();
    showSkeletons(false);
    if (library.length) {
      renderSongsGrid(allSongsFlat());
      renderHomeRails();
    } else {
      els.albumsGrid.innerHTML = '<div class="no-results">No songs found yet</div>';
    }
    bindEvents();
...
function startSupabasePolling(){
  setInterval(async () => {
    ...
    library = supabaseAlbum ? [supabaseAlbum] : [];
    publishLibrary();
    if (!els.searchInput.value.trim()) renderSongsGrid(allSongsFlat());
    renderHomeRails();
  }, SUPABASE_REFRESH_MS);
}
...
function renderSongsGrid(tracks){
  ...
  tracks.forEach(t => {
    ...
    card.innerHTML = `
      ...
      <div class="card-label">${escapeHtml(t.song.title)}</div>
      <div class="card-count">${escapeHtml(t.song.artist)}</div>
    `;
...
function bindEvents(){
  els.searchInput.addEventListener('input', (e) => {
    ...
    if (!q) { renderSongsGrid(allSongsFlat()); return; }
    const filtered = allSongsFlat().filter(t =>
      t.song.title.toLowerCase().includes(q) || t.song.artist.toLowerCase().includes(q)
    );
